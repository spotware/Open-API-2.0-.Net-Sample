...
public sealed partial class ProtoOASymbol : pb::GeneratedMessage<ProtoOASymbol, ProtoOASymbol.Builder> {
  ...
  public override void WriteTo(pb::ICodedOutputStream output) {
    ...
    if (hasPipPosition) {
      output.WriteInt32(3, field_names[14], PipPosition);
    }
    ...
    if (hasMinCommission) {
      output.WriteInt64(21, field_names[10], MinCommission);
    }
    if (hasMinCommissionType) {
      output.WriteEnum(22, field_names[12], (int) MinCommissionType, MinCommissionType);
    }
    if (hasMinCommissionAsset) {
      output.WriteString(23, field_names[11], MinCommissionAsset);
    }
